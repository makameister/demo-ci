    ...
    stages {
        ...
        stage('Push to Nexus') {
            steps {
                ...
                '''
                sh "curl -v -u jenkins:jenkins --upload-file build/release.tar.gz http://nexus:8081/repository/php-dev/packages/upload/demo-ci/0.0.${currentBuild.number} || exit 0"
            }
        }
    }
}