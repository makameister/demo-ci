        ...
        stage('Push to Nexus') {
            steps {
                sh '''
                    ...
                    curl -v -F r=release -F g=com.acme -F a=widget -F v=1.0 -F p=tar  -F file=@./release.tar -u jenkins:jenkins http://nexus:8081/nexus/content/repositories/releases/release.tar
                '''
            }
        }
    }
